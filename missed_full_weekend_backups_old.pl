...
print "Determing regular/VADP, Prod/Non Prod, Scheduled/Not scheduled\n";
foreach $val (@return) {
   chomp $val;
   $val =~ s/\;//;
   next if $val =~ /^\s*$/;
...
print "Determing all clients from the last three weeks\n"; 
(@backup_clients)=`/usr/sbin/mminfo -xc, -s $backup_server -r client -q 'savetime>three weeks ago' | /usr/bin/sort | /usr/bin/uniq`;
foreach $client (@backup_clients) {
   chomp $client;
   $client = lc($client);
   $grouper{$client} = 1;
}
print "Determining clients with full backups since last Friday\n"; 
#/usr/sbin/mminfo -xc, -a -o ct -r client,savetime,name -q 'savetime>last friday,level=full'
(@backup_full ) = `/usr/sbin/mminfo -xc, -a -o ct -s $backup_server -r 'client,savetime,totalsize'  -q 'savetime>last friday,level=full' | /usr/bin/sort | /usr/bin/uniq`;
undef %full_backup;
...
#print "Begin server size\n";
#foreach $val (sort keys %server_size) {
#   print "Server = $val, size=$server_size{$val}\n";
#}
         ...
         if ($ping =~ /up/) {
            if ($port =~ /Client listening/) {
               if (defined $scheduled_backup{$client}) {
                  if ($scheduled_backup{$client} == 1) {
                     $lower = lc($group{$client});
                     next if $lower =~ /decom/;
                     if ( $lower  =~ /prod/) {
                        if ( $backup_command{$client} == 1) {
                          $return = append_group('Automated-PROD-VADP-Reruns',$client);
                          #$return = append_group('ADHOC-Full-2',$client);
                        } else {
                          $return = append_group('Automated-PROD-Reruns',$client);
                        }
                        print "****PROD Client $client, no full backups since last Friday, $ping, $dns, $port\n";
                     } else {
                        if ( $backup_command{$client} == 1 ) {
                          $return = append_group('Automated-NON-PROD-VADP-Reruns',$client);
                        } else {
                          $return = append_group('Automated-NON-PROD-Reruns',$client);
                          #$return = append_group('ADHOC-Full',$client);
                        }
                  ...
                  print "    ***Scheduled backups not defined for client $client, client was previously deleted\n";
               }
            } else {
...
sub append_group {
    ...
    foreach $clnt (@clients) {
      chomp $clnt;
      print "Client=$clnt added to group $group\n";;
