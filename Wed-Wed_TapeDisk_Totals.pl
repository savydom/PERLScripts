...
(@return) = `/usr/sbin/mminfo -s $server -xc, -r volume,type -q 'savetime>$start_time,savetime<$end_time'`;
$volumes{'LTO Ultrium-5'} = 0;
$volumes{'adv_file'} = 0;
my $volume;
foreach $val (@return) {
   next if $val =~ /volume,type/;
   chomp $val;
   #print "Val=$val\n";
   ($volume,$type) = split(/,/,$val);
   $volumes{$type} += 1
}
if (defined $ARGV[1]) {
   print "$volumes{'LTO Ultrium-5'},$volumes{'adv_file'}\n";
} else {
