...
foreach $val (@return) {
  ...
  if ($val =~ /^\s+\d+\: /) {	# Found a Slot Number
     ($jslot) = ($val =~ /^\s+(\d+)\:/);
     #if ($val =~ /^\s+\d+\:\s+\S+\*/) {		# look for * on tape name meaning only have barcode label
     if ($val =~ /\-\*/) {		# look for * on tape name meaning only have barcode label
        ...
        if ($val =~ /UNN/) {
           ...
           print "\n\n******WARNING******: Found an asterisk tape $jtape and skipping, manually label into correct pool\n";
           next;
        }
     } else {
...
if ( ($added_percentage < 0) || ($added_percentage > 50) ) {goto PERCENT};
...
if ($really_need_slots > 0) {
   ...
   foreach $volume ( sort { $jexpiration{$a} <=> $jexpiration{$b} } keys %jexpiration) {
      ...
      $test = $tpool{$volume};
      if ($test =~ /SSCNOLALTO5/) {
         #print "Found volume Pool=$test, Volume=$volume, Expiration=$jexpiration{$volume}, Days AGo=$days_ago\n";
         $eject_lista{$volume} = 1;
         $really_need_slots -= 1;
      }
      if ($really_need_slots < 1) {goto GOTENOUGH};
