...
#$filename = "/home/scriptid/scripts/BACKUPS/daily_reports/$server\_full\_$date";
...
(@return) = `/usr/sbin/mminfo -s $server -r totalsize -q 'level=full,savetime>$start_time,savetime<$end_time'`;
$total = 0;
foreach $val (@return) {
   chomp $val;
   $total += $val;
