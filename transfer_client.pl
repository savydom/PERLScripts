...
$dname{'sscprodeng'} = 1;
$dname{'sscustno1'} = 1;
#foreach $val (sort keys %dname) {
#   print "Dname: $val\n";
#}

$nsrpass = ". type:NSR client'\n'show name\\;scheduled backup\\;group\\;action'\n'print";
...
foreach $val (@return) {
   ...
   if ($val =~ /\s+name:/) {
      ...
      $val =~ s/\;//;
      $ggg = $val;
      # Is it enabled on the source
      if ($sb =~ /Enabled/) {
         #print "Name:$name, SB:$sb, Group:$ggg\n";
         # Does it already exist on the destination
         if (!defined $dname{$name}) {
            if ($ggg !~ /DECOM/) {$group{$name} = $ggg};
         } else {
            print "Skipped=$name\n";
         }
      }   
...
#foreach $val (sort keys %group) {
#   print "Client:$val, Group:$group{$val}\n";
#}
#exit;      
$count = 0;
...
foreach $client (sort keys %group) {
